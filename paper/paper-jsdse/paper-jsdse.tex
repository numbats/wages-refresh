...
As mentioned previously, the aim of this paper is to refresh the high school dropouts wages used in textbook (Singer and Willett 2003) as similar as possible. Hence, in this section, we will compare how close the \texttt{wages\_hs\_do}, henceforth refreshed data, to the textbook data. It is also worth noting that the textbook data is reused from Murnane, Willett, and Boudett (1999)'s paper. Hence, it is not directly extracted from the NLSY79 database. Singer and Willett (2003) do not state the time frame covered by the data. However, from Murnane, Willett, and Boudett (1999)'s paper, we find that the textbook data cover the survey period from 1979-1994. Hence, for comparison purposes, we filter the refreshed data only until 1994.

In the absence of highs chool dropouts criteria in the textbook data, we create a clearer criteria of high school dropouts as mentioned in Section @ref(\#target). Consequently, we do not have the same ID of high-school dropouts as in the textbook data. The textbook data has 888 IDs included, while the refreshed data has 1184 IDs. Furthermore, there are only 94 IDs included both in the textbook and the refreshed data. After observing Murnane, Willett, and Boudett (1999)'s paper, it is mentioned that the data is randomly drawn from the low income subsample of the NLSY. This is probably the reason why, even using some criteria, we cannot match the dropouts in the textbook and the refreshed data.

Further, the temporal variable used in the textbook data is experience, instead of year. The only explanation we found regarding this variable is that it is derived from the years of IDs' experience since their first job. Singer and Willett (2003) stated that this variable reflects the specific moment in each ID labor force history associated with each wage observation.
...
Regarding the wages, the textbook data express the wages as inflation adjusted with 1990 dollars as constant (Singer and Willett 2003). However, it is not stated how exactly the wages are adjusted. Note that in the refreshed data we publish in \texttt{yowie}, we do not adjust the data with inflation because the time frame covered ranged from 1979-2018. Hence, we allow flexibility to the data user to adjust, not adjust, or adjust it with different base years. For comparison purposes, we adjust the wages with the average Consumer Price Index For All Urban Consumers (CPI-U) we got from (\textbf{fed?}).

\begin{figure}

{\centering \includegraphics[width=468px]{figures/plotting-sw-do-1} 

}

\caption{Textbook data (A) and Refreshed data (B). The textbook data and refreshed data is not exactly the same because it has different temporal variables.}\label{fig:plotting-sw-do}
\end{figure}
...
\leavevmode\hypertarget{ref-MurnaneRichardJ1999DMDB}{}%
Murnane, Richard J, John B Willett, and Kathryn Parker Boudett. 1999. {``Do Male Dropouts Benefit from Obtaining a GED, Postsecondary Education, and Training?''} \emph{Evaluation Review} 23 (5): 475--503.
