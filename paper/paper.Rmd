...
```{r flow-chart, fig.cap="The stages of data cleaning from the raw data to get three datasets contained yowie. n individuals means the number of respondents included in each stage, while n observations means the number of rows in the data. The color represented the stage of data cleaning in statistical value chain (van der Loo \\& de Jonge, 2018). Pink, blue, and green represents the raw, input, and valid data, respectively.", out.width="93%"}
```
...
- **shiny app**: An web interactive `shiny` app to run a simulation to customize the weight threshold. This app could be accessed [here](https://github.com/numbats/wages-refresh/tree/main/app).
