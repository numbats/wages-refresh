...
```{r}
wages <- as_tsibble(x = wages_tidy_sc2,
                    key = id,
                    index = year,
                    regular = FALSE)
```
...
```{r p3}
p6 <- ggplot(transform(wages_feature_long,
                 feature = factor(feature, level = 
                                 c("min",
                                   "med", 
                                   "max")))) +
