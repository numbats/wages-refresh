...
load("wages_hs2020_sc1.rda")
load("wages_hs2020_sc2.rda")
```
...
bp_sc1 <- ggplot(wages_hs2020_sc1) +
  geom_boxplot(aes(x = mean_hourly_wage)) +
  facet_wrap(~year) +
  ggtitle("Scenario 1")

bp_sc2 <- ggplot(wages_hs2020_sc2) +
  geom_boxplot(aes(x = mean_hourly_wage)) +
  facet_wrap(~year) +
  ggtitle("Scenario 2")
...
```{r}
summary(wages_sc1$mean_hourly_wage)
```

```{r}
summary(wages_sc2$mean_hourly_wage)
```
