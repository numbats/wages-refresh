---
title: "Longitudinal Data Cleaning"
subtitle: "A Case of The NLSY79 Data"
author: "Dewi Lestari Amaliah, Dianne Cook, Emi Tanaka, Nicholas Tierney, Kate Hyde"
date: "24/02/2021"
output: 
...
Wild caught data are often untidy and are not free from anomaly. Hence it is should be tidied and cleaned before a further analysis conducted to prevent inappropriate usage of statistical method that may leads to a misleading result [@HuebnerMariannePhD2016Asat]. @HuebnerMariannePhD2016Asat also argued that data tidying and cleaning is the first part of Initial Data Analysis (IDA). The second and third steps are to explore the data properties and to document and report the process for the later formal analysis. This second step is actually the same with Chatfield's perspective on the goal of Initial Data Analysis [@Chatfield1985TIEo], which is to get the "feel" of the data. 

@DasuTamraparni2003Edma mentioned that data cleaning and exploration, which is the IDA notion, is a difficult task and determine 80 percent of data mining result. Despite its importance, this stage is often undervalued and neglected [@Chatfield1985TIEo]. @WickhamHadley2014TD also stated that there has been few research of the good practice of data cleaning. Furthermore, the decisions made in this stage is often unreported [@HuebnerMariannePhD2016Asat].

This paper aims to demonstrate the steps of IDA, which is tidying, cleaning, and documenting the data. We used the longitudinal data of Americans from the National Longitudinal Survey of Youth 1979 or is known as the NLSY79. Particularly, we are interested in doing the IDA to the wages and several demographic variables of the cohort whose highest grade are up to 12th grade and the high school dropouts.  

# The NLSY79

The NLSY79  is a longitudinal survey held by the U.S Bureau of Labor Statistics that follows the lives of American Youth a sample of American youth born between 1957-64. The cohort originally included 12,686 respondents ages 14-22 when first interviewed in 1979. There were three race interviewed in the survey, which are Black, Hispanic, and non-Black/non-Hispanic. In 1984 interview, 1,079 members of the military sample were no longer eligible for interview; 201 respondents randomly selected from the military sample remained in the survey. Following the 1990 interview, none of the 1,643 members of the economically disadvantaged, non-Black/non-Hispanic sample were eligible for interview. Hence, 9,964 respondents remain in the eligible samples. The surveys were conducted annually from 1979 to 1994 and biennially thereafter. Data are now available from Round 1 (1979 survey year) to Round 28 (2018 survey year).

The NLSY79 covers several topics including Education, Training & Achievement Scores; Employment; Household, Geography & Contextual Variables; Family Background; Dating, Marriage & Cohabitation, Sexual Activity, Pregnancy & Fertility; Children; Income, Assets & Program Participation; Health; Attitudes & Expectations; and Crime and Substance Use. 

# The NLSY79 Data Cleaning 

## Getting and Tidyng the Data
...
```{r demoprofile, echo = FALSE, fig.cap="Demography Profile of the NLSY79 Cohort", fig.width=5}
...
Further, we took 36 samples randomly from the data and plotted it as is seen in Figure \ref{fig:sampleplot}. It implies that not only that some observations earned an extremely high figures of wages, but some also had a very fluctuate wages, fo example in panel number 5, 7, and 11.  

```{r sampleplot, echo = FALSE, fig.cap="Some samples of observations", fig.height=4, fig.width=5}
set.seed(20210225)
...
Since we worked with longitudinal data, we should built the model for each ID, instead of the overall data. The robust mixed model is actually the best model to be employed in this case. However, this method is too computationally and memory expensive, especially for a large data set, like the NLSY79 data. Thus, the model for each ID is built utilizing the `nest` and `map` function from `tidyr` [@tidyr] and `purrr` [@purrr] respectively. After getting the weight values, we flagged the observations with the weight less than 1, and imputed their mean hourly wage with the models' predicted value.  

Figure \ref{fig:comppict} a) shows that after imputing the outliers with the models' predicted value, the highest wages value is not exceed US$200. The spikes were still observed, but are not as extreme as the original data. In Figure \ref{fig:comppict} b), although the distributions of the features are still positively skewed, we can still examine it clearly. We also learned that some IDs' have a minimum wages that is higher than others' maximum wages. 
...
```{r comppict, echo = FALSE, fig.cap="a) The Wages and b) The Feature Distribution after the Noises Treatment", fig.width=6, fig.height=4}
...
Furthermore, the robust linear regression has reduce the level of noise in the data set as is seen in Figure \ref{fig:compare}. The result is that we got a data set with the reasonable degree of fluctuation. 

```{r compare, echo = FALSE, fig.cap="Comparison between the Original and Treated Mean Hourly Wage", fig.height=4, fig.width=5}
set.seed(20210228)
