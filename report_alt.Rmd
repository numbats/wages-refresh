...
In this part, we gave some examples of how the data might be used for data analysis in text book. The first example is that we observed the relationship between wage and the respondents' demographic variables.  Using `features` function in `brolgar`, we extracted the median of the mean hourly age of each ID and examine its relationship with respondents' gender, race, and education. 
...
```{r}
ggplot(filter(wages_median_demog, hgc != "UNGRADED"), aes(x = gender, y = med, color = gender)) +
  geom_boxplot() +
  facet_wrap(~hgc, scales = "free_y") +
  theme_bw()
```
```{r}
