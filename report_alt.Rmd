...
"Open data" is data that are freely accessible, modifiable, and shareable by anyone for any purpose [@opendata]. This type of data can be useful as example data in statistical text books and for research purposes. However, open data are often, what we might call "wild data", because it requires substantial cleaning and tidying to tame it into text book shape. Hence, it should be tidied and cleaned before further analysis to prevent inappropriate usage of statistical method that may lead to a misleading result [@HuebnerMariannePhD2016Asat].  In fact, documenting the data cleaning is utterly essential as the information preservation for the downstream statistical analyses and model building [@HuebnerMariannePhD2016Asat] as well as to avoid publication bias [@HuebnerMarianne2020Haar]. Making the data cleaning process accountable and transparent is imperative. 

@HuebnerMariannePhD2016Asat considers data cleaning to be part of "initial data analysis (IDA)". The other IDA steps are to explore the data properties and to document and report the process for the later formal analysis. This is similar to Chatfield's perspective on IDA [@Chatfield1985TIEo], which is to get the "feel" of the data. @DasuTamraparni2003Edma say that data cleaning and exploration, without naming it as IDA, is a difficult task and consumes 80% of the data mining task. 

Despite its importance, this stage is often undervalued and neglected [@Chatfield1985TIEo]. There are few research papers that document the data cleaning [@WickhamHadley2014TD]. Furthermore, the decisions made in this stage often go unreported  in term that the IDA are often performed in an unplanned and unstructured way and is only shared among restricted party [@HuebnerMarianne2020Haar]. 
...
We chose the NLSY79 data because this data is one of the instances of Open Data that has been playing an important role in research in various disciplines including but are not limited to economics, sociology, education, public policy, and public health for more than a quarter of the century [@MichaelRPergamit2001DWTN]. In addition, The National Longitudinal Survey is considered as survey with high retention rates and carefully designed making it suitable for a life course research [@MichaelRPergamit2001DWTN] and [@eliznlsy]. According to @eliznlsy as of 2015, thousand of articles, and hundreds of book chapters and monographs has incorporated the NLSY79 data. Moreover, the NLSY79 is considered as the most widely used and most important cohort in the NLSY79 data sets [@MichaelRPergamit2001DWTN]. @SingerJudithD2003Alda used the wages and other variables of the NLSY79 data in their book in longitudinal data analysis. This is also being our motivation in incorporating the wages of high school cohort, specifically the high school dropouts, is that we want to refresh this data set as well as demonstrating the process taken to transform and open data to be suitable as an example data in text book or in research. 
...
```{r comppict, echo = FALSE, fig.cap = "The distribution of the mean hourly wage after treating the extreme values. Plot A portrays the pattern of mean hourly wage of high school cohort from 1979 to 2018 of each ID in US Dollar; Plot B shows the distribution of their minimum, median, and maximum value transformed to log10 scale. We can see that some observations still had reasonbaly higher wages than the others. The minimum, median, and maximum distribution is positively skewed, where some IDs' have a minimum wages that is higher than others' maximum wages.", fig.width=6, fig.height=4}
...
In this part, we gave some examples of how the data might be used for data analysis in text book. The first example is that we observed relationship between wages and education. Using `features` function in `brolgar`, we extracted the median of the mean hourly age of each ID and plotted according to highest grade completed. We found in Figure \ref{fig:eda-plot} A that the higher the education completed does not necessarily means the higher wages earned. However, people who completed 12th grade tend to have higher wage compared to the other groups. Further, respondents who only completed 4th grade happen to have median about as high as the respondents who completed 10th grade. 

Secondly, we examined the pattern of mean hourly wage corresponding to different groups of race and gender. Figure \ref{fig:eda-plot} B and C suggest that in general, the mean hourly wage median increased over the years. Regarding the race, we found that Non-Black Non-Hispanic group had a higher wages than the other groups. We can also see a reasonable gap between the wages median of Black people and the Hispanic and the Non-Black Non-Hispanic. Regarding the gender, male earn more than female, and unfortunately, the gap keep increasing over time.  
...
wages_hs2020_feature <- wages_hs2020_tsibble %>%
  features(mean_hourly_wage,
           feat_three_num) 

wages_median_demog <- left_join(wages_hs2020_feature, demographic_nlsy79, by = "id") %>%
...
```{r eda-plot, echo = FALSE, fig.height=6, fig.width=6, fig.cap="The example of EDA plot. Plot A shows the pattern of the mean hourly wage median against the highest grade completed. It shows that the higher eductaion completed does not necessarily means the more wages earned. Plot B and C show the mean hourly wage median over the years based on the race and gender respectively. Generally, the respondents experienced a positive trend over time despite of what their race and gender."}
hgc_plot <- ggplot(filter(wages_median_demog, hgc != "UNGRADED"), aes(y = hgc, x = med)) +
  geom_boxplot() +
  theme(axis.text.y = element_text(size = 6),
        axis.title = element_text(size = 9)) +
  ggtitle("A)") +
  xlab("median of mean hourly wage (US$)") +
  ylab("highest grade completed") 

wages_hs2020_grouped <- wages_hs2020 %>%
  group_by(race, year) %>%
  summarise(median = median(mean_hourly_wage))

race_plot <- ggplot(wages_hs2020_grouped) +
  geom_line(aes(x = year,
                y = median,
                group = race,
                colour = race)) +
