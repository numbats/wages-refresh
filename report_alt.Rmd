...
In this part, we gave some examples of how the data might be used for data analysis in text book. We used `brolgar` [@brolgar] to perform exploratory data analysis. In first example, 
...
set.seed(31251587)
wages_hs2020_white <- wages_hs2020_tsibble %>%
  filter(race == "NON-BLACK, NON-HISPANIC") %>%
  sample_n_keys(size = 60)

set.seed(31251587)
wages_hs2020_black <- wages_hs2020_tsibble %>%
  filter(race == "BLACK") %>%
  sample_n_keys(size = 60)


set.seed(31251587)
wages_hs2020_hisp <- wages_hs2020_tsibble %>%
  filter(race == "HISPANIC") %>%
  sample_n_keys(size = 60)


wages_hs2020_sample_race <- bind_rows(wages_hs2020_white, wages_hs2020_black, wages_hs2020_hisp)

ggplot(wages_hs2020_sample_race, 
       aes(x = year,
           y = mean_hourly_wage,
           group = id,
           color = race)) + 
  geom_line(alpha = 0.4) +
  facet_strata(along = race) +
  theme(legend.position = "bottom")
```


```{r wages-slope, echo = FALSE}
