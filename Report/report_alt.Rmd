...
# transform the data type and year for modeling
wages_model <- wages_hs2020_tsibble %>%
...
# build GAM model
wages_fit <- lmer(
  ln_wages ~ year0 + hgc_i + gender + (1|id),
  data = wages_model
)

wages_fit_aug<- wages_model %>%
  add_predictions(wages_fit, var = "pred") %>%
  add_residuals(wages_fit, var = "resid") 

slope_only <- wages_slope %>%
  dplyr::select(id, .slope_year) %>%
  arrange(desc(.slope_year)) %>%
  slice_head(n = 12)

wages_fit_aug2 <- wages_fit_aug %>%
  filter(id %in% slope_only$id)
