...
```{r demoprofile, echo = FALSE, fig.cap="Demography Profile of the NLSY79 Cohort", fig.width=6}
...
```{r featureplot, echo = FALSE, fig.cap = "a. The mean hourly wage of cohort whose highest grade completed is up to 12th grade, b. Three features of the cohort", fig.width=6, fig.height=4}
wages_hs2020 <- as_tsibble(x = wages_demog_hs,
                    key = id,
                    index = year,
                    regular = FALSE)
...
Further, we took 36 samples randomly from the data and plotted it as is seen in Figure \ref{fig:sampleplot}. It implies that not only that some observations earned an extremely high figures of wages, but some also had a very fluctuative wages, fo example in panel number 5, 7, and 11.  

```{r sampleplot, echo = FALSE, fig.cap="Some samples of observations"}
set.seed(20210225)
