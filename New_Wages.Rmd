...
hours_wages <- hours_wages %>%
  mutate(rate_per_hour_adj = ifelse(as.numeric(year) < 1994 & rate_per_hour > 1000, 1000,
                                    ifelse(as.numeric(year) >= 1994 & rate_per_hour > 5000, 5000, rate_per_hour)))
```
...
no_job <- hours_wages %>%
  filter(!is.na(rate_per_hour_adj)) %>%
  group_by(id, year) %>%
  summarise(no_jobs = length(rate_per_hour_adj))
...
mean_hourly_wage <- 
  ...
  mutate(wages = ifelse(no_jobs == 1, rate_per_hour_adj/100,
                        weighted.mean(rate_per_hour_adj, hours_work, na.rm = TRUE)/100)) %>%
  #give the flag if it the weighted mean
  mutate(flag1 = ifelse(!is.na(wages) & no_jobs != 1, 1,
                        0)) %>%
  #calculate the arithmetic mean for the na
  mutate(wages = ifelse(is.na(wages), mean(rate_per_hour_adj)/100,
                        wages))
